# This source code is the improved dbscan clustering algorithm from Attractors.jl,
# published and discussed in Chaos 33, 073151 (2023), https://doi.org/10.1063/5.0159675
# Here the algorithm is wrapped in the `ClusteringAPI` interface.
...
function optimal_radius_dbscan_silhouette(features, min_neighbors, metric,
    ...
    for i in eachindex(ϵ_grid)
        clusters = Clustering.dbscan(dists, ϵ_grid[i]; min_neighbors, metric = nothing)
